...
class SproxydClient {
    ...
    _createRequestHeader(method, headers, key, params, log) {
        ...
        const realPath = key ? `${this.path}${key}` : this.path;
        return {
    ...
    _handleRequest(method, stream, size, key, log, callback, params) {
        ...
        if (stream) {
            ...
            const request = _createRequest(req, log, (err, response) => {
                ...
                const Method = method;
                log.info('createRequest cb', { method: Method,
                    hdr: response.headers });
                // log.info('createRequest cb', { method, response.headers });
                const realKey = response.headers['scal-key'] ?
                    response.headers['scal-key'] : newKey;
                // We return the key
                log.info('stored to sproxyd', { host, key: realKey,
                    statusCode: response.statusCode });
                return callback(null, realKey);
            });
            request.on('finish',
    ...
    put(stream, size, params, reqUids, callback, keyScheme) {
        ...
        this._failover('POST', stream, size, keyScheme, 0, log, (err, key) => {
            if (err) {
                return callback(err);
            }
    ...
    putEmptyObject(keyScheme, metadata, reqUids, callback) {
        ...
        this._failover('POST', null, 0, keyScheme, 0, log, (err, key) => {
            if (err) {
                return callback(err);
            }
            return callback(null, key);
